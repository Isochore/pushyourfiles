...
function verifUpload(){
    ...
    if ((($fileSize > 0) && ($fileSize < 2048576)) == 1){
        ...
        $filename = $_SERVER["DOCUMENT_ROOT"].'/'.'pushyourfiles/cloud/' .$full.'.'.$ext;

        if(!file_exists($filename)) {
        $resultat = move_uploaded_file($_FILES['fichier']['tmp_name'],$_SERVER["DOCUMENT_ROOT"]."/".'pushyourfiles/cloud/' .$full.'.'.$ext);
    } else {
        while(file_exists($_SERVER["DOCUMENT_ROOT"]."/".'pushyourfiles/cloud/' .$full.'.'.$ext)) {
            $ms = round(microtime(true) * 1000);
            $id = sha1(uniqid(rand(), true));
            $full = $ms . '-' . $id;
        }
        ...
        if ($resultat){
            ...
            $dlLink = 'https://julesp.promo-23.codeur.online/pushyourfiles/download/show/' . $full;
